...
public class Bag<E> implements ImmutableBag<E> {
	...
	public E remove(int index) throws ArrayIndexOutOfBoundsException {
		E e = data[index]; // make copy of element to remove so it can be
							// returned
	 ...
	 * it is present.
	 * <p>
	 * If the Bag does not contain the element, it is unchanged. It does this by
	...
	/**
	 * Returns the element at the specified position in Bag. This method ensures
	 * that the bag grows if the requested index is outside the bounds of the
	...
	/**
	 * Returns a copy of the backing array cast to the 
	 * desired type, NOT THE ORIGINAL ARRAY
	 * 
	 * @return
	 * 		a copy of the backing array cast to type
	 */
	public E[] copyOfDataInto(E[] intoArray) {
        if(intoArray.length != size)
            throw new RuntimeException("Array is not exactly as long as the amount of data!");
        System.arraycopy(data, 0, intoArray, 0, size);
		return intoArray;
	}
